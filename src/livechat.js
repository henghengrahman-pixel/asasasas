...
export class LiveChatClient {
  ...
  async listChats() {
    ...
    for (const body of candidates) {
      try {
        firstPage = await this.call('list_chats', body);
    ...
    // LiveChat cursors carry the original filters/sort/limit. For subsequent pages the API
    // requires page_id by itself; resending filters/limit/sort_order with page_id returns 422.
    // Track cursor ids to guarantee a malformed/repeated provider cursor cannot loop forever.
    const seenPageIds=new Set();
    let page=firstPage;
    while (page?.next_page_id) {
      const pageId=String(page.next_page_id);
      if (seenPageIds.has(pageId)) break;
      seenPageIds.add(pageId);
      page=await this.call('list_chats', { page_id: pageId });
      addPage(this.normalizeChatList(page).items);
    }

    return { ...firstPage, _normalizedChats: chats, _listSource: firstNormalized.source };
  }
